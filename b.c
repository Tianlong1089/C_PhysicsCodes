...
for (int i = 0; i < T-1; i++) {
  ...
  malla[j][i+1]= malla[j][i+1]=0.5*(malla[j+1][i] + malla[j-1][i]) - v*0.5*(DT/DX)*(malla[j+1][i] - malla[j-1][i]);
}
malla[j][i]=malla[j][i+1];
 aux=aux+1;
}
//Impresion
FILE *file=fopen("bdata.txt","w");
if(file != NULL)
{
aux=1;
for (int  i = 0; i < T-1; i++)
...
fprintf(gnuplot, "set multiplot layout 1,1 title 'Evolucion con Metedo METODO Lax-Friedrich' \n");


fprintf(gnuplot, "set xlabel 'Posicion X' \n");
fprintf(gnuplot, "set ylabel 'Posicion U n+1' \n");
fprintf(gnuplot, "unset key \n");
fprintf(gnuplot, "plot 'bdata.txt' using 1:2 with lines lt 6  \n");
}
